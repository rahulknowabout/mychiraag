...
foreach( $arraystatus as $arraystatusK => $arraystatusV ){
	...
	$allSet = $ketObj->runquery("UPDATE", "", "ketechord_".$_SESSION['vid'], $ketechstatus, "WHERE id=".$arraystatusK	 );
}
foreach( $arraypack as $arraypackK => $arraypackV ){
	
	$ketechpack['pack_staff'] = $arraypackV;
		
	$allSet = $ketObj->runquery("UPDATE", "", "ketechord_".$_SESSION['vid'], $ketechpack, "WHERE id=".$arraypackK	 );
}
foreach( $arraydel as $arraydelK => $arraydelV ){
	
	$ketechdel['del_staff'] = $arraydelV;
		
	$allSet = $ketObj->runquery("UPDATE", "", "ketechord_".$_SESSION['vid'], $ketechdel, "WHERE id=".$arraydelK	 );
}			
	...
	header( "Location: index.php?v=".$_POST['c']."" );
}	
		/*if( $allSet > 0 )
		{
