...
if( isset( $_POST['idcc'])  ){
	...
	$whe = "where cparent = ".$_POST['idcc']." AND cstatus = 1";
	...
	if( isset( $allCat ) && count( $allCat ) > 0 && !isset( $flag ))
	{
	 foreach( $allCat as $CatParentKey => $CatParentValue ) {  			
	  ...
	  { $selbox = '<select name="Catsecond[]" onchange="CatAjax(this.value,\'ch\')" class = "form-control">
	  	<option value=""> Chosse Sub Category </option>'.$opt.'</select>';
...
if( isset( $_POST['chk'] ) && $_POST['chk'] == "vemail" )
{
	$uid	=	$_POST['uid'];
	$vemail	=	$_POST['vemail'];
	$totalu = $ketObj->runquery( "SELECT", "uphone", "ketechuser", array(), " WHERE uphone='".$vemail."' AND vid<>'".$uid."'","num_rows");
	if( $totalu > 0 )
	{
		echo "yes";
	}else
...
if( isset( $_POST['chk'] ) && $_POST['chk'] == "uvemail" )
{
	$uid	=	$_POST['uid'];
	$vemail	=	$_POST['vemail'];
	$totalu = $ketObj->runquery( "SELECT", "uphone", "ketechuser", array(), " WHERE uphone='".$vemail."' AND id<>'".$uid."'","num_rows");
	if( $totalu > 0 )
	{
		echo "yes";
	}else
...
{  /* echo "<pre>";print_r( $_POST);die();*/
	...
	$vcity	=	$_POST['vcity'];
	$totalu = $ketObj->runquery( "SELECT", "area_name", "ketechvendorarea", array(), " WHERE area_name='".$areaname."' AND id<>'".$cid."' AND vcity = ".$vcity."","num_rows");
	if( $totalu > 0 )
	{
		echo "yes";
	}else
...
if( isset( $_POST['chk'] ) && $_POST['chk'] == "deliverytime" )
{  /* echo "<pre>";print_r( $_POST);die();*/
	$id	=	$_POST['id'];
	$del_from =    $_POST['del_from'];
	$del_to =      $_POST['del_to'];
	//$vemail	=	$_POST['vemail'];
	$totalu = $ketObj->runquery( "SELECT", "*", "ketechdelivery_time", array(), " WHERE from_time='".$del_from."' AND  to_time='".$del_to."' AND id<>'".$id."'","num_rows");
	if( $totalu > 0 )
	{
		echo "yes";
	}else
...
if( isset( $_POST['chk'] ) && $_POST['chk'] == "areanamev" && isset( $_POST['vcity'] ) && $_POST['vcity']!="" )
{  /* echo "<pre>";print_r( $_POST);die();*/
	$id	=	explode("/",$_POST['vcity']);
	$opt = "<option value = ''>Choose Area</option>";
	//$vemail	=	$_POST['vemail'];
	$varea = $ketObj->runquery( "SELECT", "*", "ketechvendorarea", array(), " WHERE vcity = ".$id['0']."");
	if( isset($varea) && is_array($varea) && count($varea)>0 ){
		foreach( $varea as $vareav ){
			$opt .= "<option value = '".$vareav['id']."'>".$vareav['area_name']."</option>";
